...
export function useAudioAnalyzer({
  ...
  const start = useCallback(async (deviceId) => {
    try {
      ...
      processor.onaudioprocess = (event) => {
        ...
        const inputDurationMs = (input.length / sampleRate) * 1000;
        if (rms > 0.018) {
          chunkSpeechMs += inputDurationMs;
        }
  ...
  }, [fftSize, smoothingTimeConstant, chunkDurationMs, minSpeechMs, onChunkReady, onChunkSkipped, onRecordingStateChange, analyzeAudio]);
